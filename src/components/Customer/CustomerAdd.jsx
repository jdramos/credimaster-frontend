...
const normalizeCustomerForApi = (customer, isEmployee) => {
  return {
    ...
    identity_expiration_date: toIsoDateOrNull(
...
const CustomerAdd = () => {
  ...
  const cleanedGuarantees = guarantees.map((g) => ({
    ...
    value:
      typeof g.value === "string"
        ? parseFloat(g.value.replace(/[^0-9.]/g, ""))
        : g.value,
  }));
          ...
          const cleaned = (Array.isArray(g) ? g : []).map((guar) => ({
            ...
            value:
              typeof guar.value === "string"
                ? parseFloat(guar.value.replace(/[^0-9.]/g, ""))
                : guar.value,
          }));
  ...
  const handleSubmit = (e) => {
    ...
    const vGeneral = generalInfoRef.current?.validate?.() ?? {
      ok: true,
      errors: {},
    };
    const vBusiness = businessRef.current?.validate?.() ?? {
      ok: true,
      errors: {},
    };
    const vGuarantees = guaranteesRef.current?.validate?.() ?? {
      ok: true,
      errors: {},
    };
    const vReferences = referencesRef.current?.validate?.() ?? {
      ok: true,
      errors: {},
    };
    // si evaluation no bloquea guardar, ni lo metas; si bloquea, incluilo:
    const vEvaluation = evaluationRef.current?.validate?.() ?? {
      ok: true,
      errors: {},
    };
