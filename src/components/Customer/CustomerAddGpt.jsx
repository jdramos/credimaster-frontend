...
const normalizeValidate = (result) => {
  ...
  if (result && typeof result === "object") {
    const ok = result.ok ?? result.valid ?? true;
...
const handleSubmit = (e) => {
  ...
  const mergedErrors = cleanErrors(merged);
  ...
  if (Object.keys(mergedErrors).length > 0) {
    ...
    // opcional: lista de errores en toast (1-3)
    toast.error("Corrija los errores antes de continuar..");
    // toast.error(`${firstField}: ${mergedErrors[firstField]}`);

    // focus/scroll al primer error
    setTimeout(() => focusFirstError(firstField), 150);
    return;
  }

  // ✅ setCustomer (NO mutar directo)
  setCustomer((prev) => ({
    ...prev,
    economic_activity: isEmployee ? 2 : 1,
  }));
    ...
    <ul style={{ margin: 0, paddingLeft: 18 }}>
      {Object.entries(errors).map(([k, v]) => (
        ...
        <Tabs value={activeTab} onChange={handleTabChange} variant="scrollable" scrollButtons="auto">
          ...
          <Tab label="Evaluación financiera" />
