...
const PromoterList = () => {
  ...
  const [state, setState] = useState({ open: false }); // State to store fetched data
  ...
  const columns = useMemo(
    () => [
      ...
      {
        accessorKey: "identification", //normal accessorKey
  ...
  useEffect(() => {
    const fetchApi = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          if (jsonData.errors && jsonData.errors.length > 0) {
            setAlert({
              alertType: "error",
              alertMessage:
                "Catch: Error al obtener los registros." + response.statusText,
            });
          }
        }
        ...
        setAlert({
          alertType: "error",
          alertMessage: "Catch: Error al obtener los registros." + error,
        });
        return;
      }
    };
