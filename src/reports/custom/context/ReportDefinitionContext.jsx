...
export const ReportDefinitionProvider = ({ children }) => {
  ...
  useEffect(() => {
    let active = true;
    API.get("/api/custom-reports/sources")
      .then((response) => {
        if (!active) return;
      ...
      .catch((error) => {
        ...
        setSourcesError(error.response?.data?.message || "No se pudieron cargar las fuentes de datos.");
      })
  ...
  const addCalculatedField = (calculatedField = {}) => {
    const id = calculatedField.id || `expr_${Date.now()}`;
    const name = calculatedField.name || id;
    ...
    setDefinition((prev) => ({
      ...
      calculatedFields: [
        ...(prev.calculatedFields || []),
        {
          id,
          name,
          label: calculatedField.label || "Campo calculado",
          type:
            calculatedField.type || calculatedField.returnType || "currency",
          returnType:
            calculatedField.returnType || calculatedField.type || "currency",
          format: calculatedField.format || "money",
          visible: calculatedField.visible ?? true,
          width: calculatedField.width || 120,
          align: calculatedField.align || "right",
          totalable: calculatedField.totalable ?? true,
          nodes: calculatedField.nodes || [],
        },
      ],
      fields: prev.fields.some((field) => field.name === name)
        ? prev.fields
        : [
            ...prev.fields,
            {
              ...
              align: calculatedField.align || "right",
              format: calculatedField.format || "money",
              type: calculatedField.type || "currency",
              totalable: true,
              sourceId: id,
            },
          ],
  ...
  const updateCalculatedField = (id, changes) => {
    setDefinition((prev) => ({
      ...prev,
  ...
  const removeCalculatedField = (id) => {
    setDefinition((prev) => ({
      ...prev,
      calculatedFields: (prev.calculatedFields || []).filter(
        (field) => field.id !== id,
      ),
      fields: (prev.fields || []).filter(
        (field) => field.sourceId !== id && field.name !== id,
      ),
      filters: prev.filters.filter((filter) => filter.field !== id),
      groups: prev.groups.filter((group) => group.field !== id),
      sorts: prev.sorts.filter((sort) => sort.field !== id),
      totals: prev.totals.filter((total) => total.field !== id),
    }));
  };
